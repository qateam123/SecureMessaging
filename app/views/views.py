...
def get_msgs(label=None):
    ...
    if logged_in is True:
        if request.method == 'GET':
            ...
            resp_data = json.loads(resp.text)
            links = resp_data['_links']
        return render_template("secure-messaging/messages.html", messages=resp_data['messages'], links=links, user_urn=token_data['user_urn'])
    else:
        return redirect(url_for('login'))
...
def get_msg(message_id):
    ...
    if logged_in is True:
        ...
        return render_template("secure-messaging/message.html", message=response, user_urn=token_data['user_urn'])
    else:
        return redirect(url_for('login'))
...
def get_draft(draft_id):
    ...
    if logged_in is True:
        ...
        resp = requests.get(url, headers=headers)
        response = json.loads(resp.text)
        return render_template("secure-messaging/draft.html", draft=response, user_urn=token_data['user_urn'])
    else:
        return redirect(url_for('login'))
...
def edit_draft(draft_id):
    ...
    if logged_in is True:
        ...
        form.to_input.data = resp_data['urn_to']
        form.subject_input.data = resp_data['subject']
        form.message_input.data = resp_data['body']

        if request.method == 'POST':
            url = settings.SECURE_MESSAGING_API_URL
            if request.form['submit'] == 'Send Message':
                return send_draft(request.form, url, resp_data['thread_id'], resp_data['msg_id'])
            elif request.form['submit'] == 'Save as Draft':
                return modify_draft(request.form, url, resp_data['thread_id'])

        return render_template('secure-messaging/draft-edit.html', form=form, user_urn=token_data['user_urn'])
    else:
        return redirect(url_for('login'))
...
def send_message(form, url, thread_id):
    ...
    return render_template("secure-messaging/sent-message.html", user_urn=token_data['user_urn'], code=201)
...
def save_draft(form, url, thread_id):
    ...
    resp_data = json.loads(response.text)
    logger.info(response.status_code, resp_data['msg_id'])
    return render_template("secure-messaging/save-draft.html", user_urn=token_data['user_urn'], code=201)
...
def modify_draft(form, url, thread_id):
    ...
    requests.put(url + settings.SM_SAVE_DRAFT_URL, data=json.dumps(modify_draft_data), headers=headers)
    return render_template("secure-messaging/save-draft.html", user_urn=token_data['user_urn'], code=201)


def send_draft(form, url, thread_id, msg_id):
    headers['Authorization'] = update_encrypted_jwt()
    send_draft_data = {'msg_id': msg_id, 'urn_to': form['to_input'], 'urn_from': token_data['user_urn'], 'subject': form['subject_input'],
                       'body': form['message_input'], 'thread_id': thread_id, 'collection_case': 'testCC', 'reporting_unit': 'testRU', 'survey': 'BRES'}
    requests.post(url + settings.SM_SEND_MESSAGE_URL, data=json.dumps(send_draft_data), headers=headers)
    return render_template("secure-messaging/sent-message.html", user_urn=token_data['user_urn'], code=201)
